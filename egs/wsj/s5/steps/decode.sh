...
mkdir -p $dir/log
[[ -d $sdata && $data/feats.scp -ot $sdata ]] || /root/k0/egs/Capstone_ASR/utils/split_data.sh $data $nj || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  /root/k0/egs/Capstone_ASR/steps/diagnostic/analyze_lats.sh --cmd "$cmd" $iter_opt $graphdir $dir
fi

if ! $skip_scoring ; then
  [ ! -x /root/k0/egs/Capstone_ASR/local/score.sh ] && \
    echo "$0: Not scoring because local/score.sh does not exist or not executable." && exit 1;
  /root/k0/egs/Capstone_ASR/local/score.sh --cmd "$cmd" $scoring_opts $data $graphdir $dir ||
